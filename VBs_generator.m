...
% Master figure for Polarization Maps comparison
hMaster = figure('Name', 'Polarization Maps Comparison (Basis 1, 2, 3)', 'Units', 'normalized', 'Position', [0.1 0.3 0.8 0.4]);
...
coeffA = cos(polcomponentsamplitudefactor)*exp(-1i*polcomponentsphasefactor/2);
coeffB = sin(polcomponentsamplitudefactor)* exp(1i*polcomponentsphasefactor/2);

if basisType == 1 || basisType == 2
    % Direct mapping (Default behavior)
    % If basisType = 2, setting amplitudefactor = pi/4, phase = 0
    % will correctly produce 45 degree linear polarization.
    modee1 = coeffA * modeA;
    modee2 = coeffB * modeB;
else
    % basisType = 3: Linear (H/V) + 45 degree Quarter Waveplate Physical Simulation
    % 1. Start with Mode A in H and Mode B in V
    tempJx = coeffA * modeA;
    % Phase compensation (-90 deg) for V channel to align R and L phases for horizontal output
    tempJy = coeffB * modeB * (-1i);
    
    % 2. Apply a 1/4 Waveplate with Fast Axis at 45 degrees
    % Theory: H -> (1, -i)/sqrt(2) [Right Circular], V * (-i) -> -i * (-i, 1)/sqrt(2) = (-1, -i)/sqrt(2)
    % After constant phase normalization, this matches the (1,i) [Left Circular] basis superposition.
    [modee1, modee2] = Waveplate(pi/4, pi/2, tempJx, tempJy);
end
